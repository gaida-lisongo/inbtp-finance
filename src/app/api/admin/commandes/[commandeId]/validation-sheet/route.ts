...
import { generateValidationSheetForFaculty } from "@/lib/utils/supabase/faculty-commandes";
...
export async function POST(_request: Request, context: { params: Promise<{ commandeId: string }> }) {
  try {
    ...
    const result = await generateValidationSheetForFaculty(commandeId);
    ...
    if (message === "invalid_validation_commande") {
      return new NextResponse("Cette commande n'est pas de type document.", { status: 400 });
    }

    if (message === "validation_commande_not_paid") {
      return new NextResponse("Le statut success est requis pour la generation.", { status: 400 });
    }

    if (message === "invalid_validation_document") {
      return new NextResponse("Ce document n'est pas une fiche de validation.", { status: 400 });
    }

    if (message === "validation_notes_missing") {
