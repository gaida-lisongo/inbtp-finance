...
export async function GET(request: Request, context: { params: Promise<{ product_id: string }> }) {
  try {
    ...
    if (documentCategory.trim().toLowerCase() !== "fiche de validation") {
      return NextResponse.json({ valid: false, error: "Document non conforme a une fiche de validation." }, { status: 400 });
    }
    ...
    const { data: commandesData, error: commandesError } = await admin
      ...
      .eq("status", "success")
      .in("categorie", ["documents", "document"])
