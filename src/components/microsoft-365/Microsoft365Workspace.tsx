...
export default async function Microsoft365Workspace() {
  const loadResult = await (async () => {
    try {
      ...
      return { overview, errorMessage: null as string | null };
    } catch (error) {
      return {
        overview: null as Awaited<ReturnType<typeof getMicrosoft365Overview>> | null,
        errorMessage:
          error instanceof Error ? error.message : "Impossible de charger les donnees Microsoft 365 pour ce compte.",
      };
    }
  })();

  if (!loadResult.overview) {
    return (
    ...
    <div className="col-span-12 rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
      <div className="mb-6 flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Profil, boite mail, calendrier et OneDrive du compte connecte.</p>
        </div>
            ...
            {overview.messages.length > 0 ? (
              overview.messages.map((message) => (
                <div key={message.id} className="rounded-xl bg-gray-50 p-3 dark:bg-white/[0.03]">
                  <p className="text-sm font-medium text-gray-800 dark:text-white/90">{message.subject || "Sans objet"}</p>
            ...
            {overview.events.length > 0 ? (
              overview.events.map((event) => (
                <div key={event.id} className="rounded-xl bg-gray-50 p-3 dark:bg-white/[0.03]">
                  <p className="text-sm font-medium text-gray-800 dark:text-white/90">{event.subject || "Evenement sans titre"}</p>
          ...
          <div className="space-y-3">
            {overview.files.length > 0 ? (
              overview.files.map((file) => (
                ...
                >
                  <p className="text-sm font-medium text-gray-800 dark:text-white/90">{file.name || "Fichier sans nom"}</p>
                  <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{formatDate(file.lastModifiedDateTime)}</p>
                </a>
              ))
            ) : (
