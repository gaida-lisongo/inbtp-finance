...
const AppHeader = () => {
  const { profile, isLoading, accountType } = useUserStore();
  ...
  const fetchNotifications = async () => {
    try {
      const response = await fetch(`/api/header-notifications?accountType=${accountType}`, {
        method: "GET",
        cache: "no-store",
      });
  ...
  useEffect(() => {
    if(accountType) fetchNotifications();
  }, [accountType])
